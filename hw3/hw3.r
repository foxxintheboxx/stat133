...
plot(times_sec ~ new_year, data = wr1500m, type = "s", xlim = c(1890, 2014))
lines(c(wr_1998$new_year, 2014), c(wr_1998$times_sec, wr_1998$times_sec), type="l")
...
abline(a = NULL, b = NULL, h = NULL, v = wr_1944$new_year, col = "grey")
abline(a = NULL, b = NULL, h = NULL, v = wr_1998$new_year, col = "grey")
text(x=c(wr_1944$new_year, wr_1998$new_year),
	y= c(wr_1944$times_sec, wr_1998$times_sec), 
	labels=c(as.character(wr_1944$athlete),
		as.character(wr_1998$athlete)), 
	cex = 0.7, 
	adj = c(-0.1, -0.5))
...
plot(times_sec ~ new_year, 
	data = wr1500m, 
	type = "s",
	xlim = c(1890, 2014),
	xlab = "Years",
	ylab = "Time in Seconds", 
	main = "The World Record 1500m versus Years")
lines(c(wr_1998$new_year, 2014), c(wr_1998$times_sec, wr_1998$times_sec), type="l")
abline(a = NULL, b = NULL, h = NULL, v = wr_1944$new_year, col = "grey")
abline(a = NULL, b = NULL, h = NULL, v = wr_1998$new_year, col = "grey")
text(x=c(wr_1944$new_year, wr_1998$new_year),
...
#TODO
SO2012Ctry <- SO2012Ctry
symbols(log(SO2012Ctry$pop), log(SO2012Ctry$GDPPP), circles = SO2012Ctry$Total )
...
symbols( 
	log(SO2012Ctry$pop[SO2012Ctry$Total > 0]),
	log(SO2012Ctry$GDPPP[SO2012Ctry$Total > 0]),
	circles = SO2012Ctry$Total[SO2012Ctry$Total > 0],
	inches = 0.7)
text(log(SO2012Ctry$GDPPP[SO2012Ctry$Total == 0]), log(SO2012Ctry$pop[SO2012Ctry$Total == 0]), ".")
...
#CHECK
top5 <- SO2012Ctry[order(SO2012Ctry$Total, na.last = TRUE, decreasing = TRUE)[1:5],]
symbols(
	log(SO2012Ctry$pop[SO2012Ctry$Total > 0]), 
	log(SO2012Ctry$GDPPP[SO2012Ctry$Total > 0]),
	circles = SO2012Ctry$Total[SO2012Ctry$Total > 0],
	inches = 0.7,
	main = "Countries and their Total Medals",
	xlab = "log(Population)", 
	ylab = "log(GDPPP)")
points(log(SO2012Ctry$pop[SO2012Ctry$Total == 0]), log(SO2012Ctry$GDPPP[SO2012Ctry$Total == 0]),  pch = ".")
text(log(top5$pop),log(top5$GDPPP),  as.character(top5$ISO))
...
symbols(wonMedal$longitude, 
 	wonMedal$latitude, 
 	circles = wonMedal$Total, inches = 0.3, add = TRUE, bg = myGold)
...
n.woman <- nrow(athletes[athletes$Sex == "F",])
# What proportion of the participants were women?
frac.women <- n.woman / n.athletes
