...
email.address <- "foxxintheboxx@berkeley.edu"
...
# Session > Set W orking Directory > To Source File Location
...
# Each vector of length 100 of Poisson (hint:rpois) random variables with mean 5
  set.seed(71)
l <- lapply(seq(1,12,1), function (col) { rpois(100, 5)})
...
f2 <- family[family$gender == "m" & family$heigh >=72,]
...
f4 <- family[which(min(family$age) == family$age),]
...
cols = sapply(iris$Species, function(x) { 
  if (x == "virginica") {
    return("red")
  }
  if (x == "setosa") {
...
plot(iris$Petal.Length, iris$Petal.Width, xlab = "Petal Length", ylab = "Petal Width", col = cols)

# [3 pt]
# Make a scatterplot of ( sepal length / petal length) as a function of index (order)
# Color the plotting symbol by Species (any 3 colors)

plot(seq(1, nrow(iris), 1), iris$Sepal.Length / iris$Petal.Length,  col = cols)
...
plot(infants$gestation, infants$bwt, pch = "*", col = "red", ylab = "the birthweight of an infant", xlab = "the gestational days for a mother")
abline(v = 259)
# [6 pts]
...
# Add x-axis labels
#TODO
hist(infants$age, freq = FALSE, plot = TRUE, ylim = as.double(c(0, 0.07)),col = "blue", xlab = "mothers age")
...
mean.long.cache <- lapply(Cache500, function(x) {
  if (length(x) >= 50) {
    return(mean(x))
  } else {
    return(NA)
...
max.petal.width <- unlist(lapply(species, function(name) { 
  subset = iris[iris$Species == name,]
  max(subset$Petal.Width)
  }))
...
firstColToNames <- function(data){
  if (class(data) == "data.frame") {
    data = as.matrix(data)
    rownames = data[,1]
    data = data[,2 : ncol(data)]
    data = matrix(data, nrow = nrow(data), ncol = ncol(x), dimnames = list(rownames))
...
dice_sum <- function(k, B){
  ...
  for (i in 1:B) {
    s = 0
    for (i in 1:k) {
      s = s + sample(1:6, 1, prob = rep(1/6, 6))
    }
    total = c(total, s)
  }
  return(total)
...
ave.diceRoll <- c(0,0,0,0)
sd.diceRoll <- c(0,0,0,0)
Bvec <- c(20, 100, 1000, 5000)

for (i in 1:4) {
  result = dice_sum(2, Bvec[i])
  hist(result, main=paste("histogram for B=", Bvec[i]), xlab = "sum of dice roll")
  ave.diceRoll[i] = mean(result, na.rm = TRUE)
  sd.diceRoll[i] = sd(result, na.rm = TRUE)
}
...
text3 <- phrases[grep("[at]{3,}", phrases)]

# [3 pts]
# Create a vector [tests] that is of length 200 and has the entries
# "test1", "test2", ..., "test200"

tests <- unlist(lapply(1:200, function(num) { paste("test", num, sep = "")}))
